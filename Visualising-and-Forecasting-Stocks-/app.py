...
item1 = html.Div(
    [
        html.P("Visualising and Forecasting Stocks", className="start"),

        html.Div([
            # stock code input
            dcc.Input(id='stock-code', type='text', placeholder='Enter Company Name'),
        ...
        html.Div([
            # Stock price button
            html.Button('Get Stock Graph', id='stock-price-button'),

            # Indicators button
            html.Button('Get Moving Avg', id='indicators-button'),
...
def forecast(n, n_days, stock_code):
    ...
    if stock_code == '':
        raise PreventUpdate
    fig = prediction(int(n_days)+1, stock_code)
    return dcc.Graph(figure=fig)
